...
func SearchGame(ctx *gin.Context, request interface{}) (int, errs.IError, interface{}) {
	...
	if req.Param == nil {
		return http.StatusOK, errs.New(constants.ErrParam, "nil param"), nil
	}
	if req.GetParam().GetOffset() > constants.MaxGameSearchOffset ||
		req.GetParam().GetLimit() > constants.MaxGameSearchLimit {
		return http.StatusOK, errs.New(constants.ErrParam, "size out of limit"), nil
	}
	...
	if err != nil {
		return http.StatusOK, errs.Wrap(constants.ErrParam, "param translate fail", err), nil
	}
	...
	if err != nil {
		return http.StatusOK, errs.Wrap(constants.ErrES, "search es fail", err), nil
	}
